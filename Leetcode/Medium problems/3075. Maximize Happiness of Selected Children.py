...
# Solution 1 (First accepted attempt)
class Solution:
    def maximumHappinessSum(self, happiness: List[int], k: int) -> int:
        ...
        for i in range(k):
            new_happy = happiness[i]
            result += new_happy - i if (new_happy - i) > 0 else 0

        return result
    

# Solution 2 (List comprehension short solution)
class Solution:
    def maximumHappinessSum(self, happiness: List[int], k: int) -> int:
        happiness.sort(reverse=True)
        ...
        # func = lambda x: x - 1 if x else 0

        # result = 0
        # for _ in range(k):
        #     current_max = max(happiness)
        #     result += current_max
        #     idx = happiness.index(current_max)
        #     happiness = list(map(func, happiness))
        #     happiness[idx] = 0
